...
describe("main workflow", () => {
    ...
    it("supports local repos", async () => {
        ...
        const snapshot = JSON.parse(readFileSync(resolve(__dirname, "../mocks/ci-test-repo.json"), { encoding: "utf-8" }));
        ...
        const repository = await openRepository(repoUrl, {
            nodeishFs: fs,
            branch: "test-symlink",
        });
    ...
    it("can open repo with lazy clone/checkout", async () => {
        ...
        expect(await lazyRepo.nodeishFs.readFile("./.npmrc", { encoding: "utf-8" })).toBe("engine-strict=true\n");
        expect(await lazyRepo.nodeishFs._isPlaceholder("./README.md")).toBe(true);
        ...
        expect(await lazyRepo.statusList({
            includeStatus: ["materialized"],
        })).toStrictEqual([
            [
                ".gitignore",
    ...
    it("uses nested gitignore files correctly", async () => {
        ...
        const status = (await repo.statusList({ includeStatus: ["materialized", "ignored"] })).filter(([name]) => !name.startsWith(".git/"));
        expect(status).toStrictEqual([
            [
                ".gitignore",
                "unmodified",
    ...
    it("uses standard commit logic for non whitelisted repos", async () => {
        const nonWhitelistedRepo = await openRepository("https://github.com/janfjohannes/unicode-bug-issues-1404", {
            nodeishFs: createNodeishMemoryFs(),
        });
    ...
    it("finds initial commit of repo", async () => {
        const fs = createNodeishMemoryFs();
        const snapshot = JSON.parse(readFileSync(resolve(__dirname, "../mocks/ci-test-repo-no-shallow.json"), {
            encoding: "utf-8",
        }));
        fromSnapshot(fs, snapshot);
        const repo = await openRepository("file://", {
