...
export const rewrite = ({ root: ast, code, originalCode, translations, }) => {
    ...
    for (const [element_name, attribute_translations] of Object.entries(translations)) {
        ...
        for (const element of elements) {
            if (hasSpreadAttribute(element)) {
                ...
                const newSpreadAttributeString = c.spreadAttr(`${i("handle_attributes")}(${attributeObjectSource}, ${uneval(attribute_translations)})`);
                ...
                for (const element_translations of Object.entries(translations)) {
                    ...
                    for (const { attribute_name, lang_attribute_name } of attribute_translations) {
                        ...
                        if (!attribute)
                            continue;
                        if (getAttributeByName(element, NO_TRANSLATE_ATTRIBUTE))
                            continue;
                        const langAttribute = lang_attribute_name
                            ? getAttributeByName(element, lang_attribute_name)
                            : undefined;
                        const newAttributeCode = c.attribute(attribute_name, `${i("translateAttribute")}(
    ...
    for (const element of svelteElements) {
        ...
        if (!thisAttribute)
            continue;
        const thisValue = typeof thisAttribute === "string"
            ? c.str(thisAttribute)
        ...
        if (hasSpreadAttribute(element)) {
            ...
            for (const [element_name, attribute_translations] of Object.entries(translations)) {
                value = c.ternary(c.eq(thisValue, c.str(element_name)), `${i("handle_attributes")}(${attributeObjectSource}, ${uneval(attribute_translations)})`, value);
            }
            ...
            for (const [element_name, attribute_translations] of Object.entries(translations)) {
                for (const { attribute_name, lang_attribute_name } of attribute_translations) {
                    ...
                    const langAttribute = lang_attribute_name
                        ? getAttributeByName(element, lang_attribute_name)
                        : undefined;
                    const newAttributeCode = c.attribute(attribute_name, c.ternary(c.eq(thisValue, c.str(element_name)), `${i("translateAttribute")}(${attrubuteValuesToJSValue(attribute.value, originalCode)},${langAttribute
                        ? attrubuteValuesToJSValue(langAttribute.value, originalCode)
                        : "undefined"})`, attrubuteValuesToJSValue(attribute.value, originalCode)));
                    //replace the attribute with the new attribute
                    code.overwrite(attribute.start, attribute.end, newAttributeCode);
                }
            }
    ...
    const before = [
        `import { getTranslationFunctions as ${i("getTranslationFunctions")} } from '@inlang/paraglide-sveltekit/internal';`,
    ];
    const after = [
...
function getAttributesObject(element, originalCode) {
    const attributeEntries = [];
    const replacedAttributes = new Set();
    for (const attribute of element.attributes) {
        switch (attribute.type) {
            case "Attribute": {
                attributeEntries.push(`${c.str(attribute.name)} : ${attrubuteValuesToJSValue(attribute.value, originalCode)}`);
                replacedAttributes.add(attribute);
                break;
            }
            case "Spread": {
                const code = originalCode.slice(
                //@ts-ignore
 ...
 * @param originalCode
 */
function hasAlreadyBeenRewritten(originalCode) {
